...
cur.execute("DROP TABLE IF EXISTS Checking_accounts")
...
cur.execute("CREATE TABLE IF NOT EXISTS Users(id INTEGER PRIMARY KEY AUTOINCREMENT, loginId INTEGER REFERENCES Login(id), email TEXT REFERENCES Login(email), name TEXT, dateOfBirth TEXT, savingsAccountNo INTEGER, checkingAccountNo INTEGER, address TEXT )")
cur.execute("CREATE TABLE IF NOT EXISTS Checking_accounts(id INTEGER PRIMARY KEY AUTOINCREMENT, checkingAccountNo INTEGER REFERENCES User(checkingAccountNo), currentBalance REAL, routingNo INTEGER)")
...
cur.execute('INSERT INTO Users(loginId, email, name, dateOfBirth, savingsAccountNo, checkingAccountNo, address) VALUES(?, ?,?,  ?, ?, ?, ?)', (1, 'test@gmail.com', 'testname', '01/02/1990', '1111111111', '2222222222', '1000 Palo Verde Road, UCI, Irvine'))
cur.execute('INSERT INTO Checking_accounts(checkingAccountNo, currentBalance, routingNo) VALUES(?, ?, ?)', (2222222222, 5000, 12345))
